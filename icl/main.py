...
def estimate_log_c1_from_pool(log_r_pool: List[float], quantile: float = 0.05) -> float:
    if not log_r_pool:
        return -80.0
    return float(np.quantile(np.array(log_r_pool, dtype=np.float64), quantile))


def estimate_log_c2_per_token(model, tok, trec, phi_task, sample_prompts: int = 32, device=None, q: float = 0.01) -> float:
    if device is None:
        device = next(model.parameters()).device
    per_token_logs = []
    with torch.no_grad():
        for _ in range(sample_prompts):
            ...
            logprobs = torch.log_softmax(out.logits, dim=-1)[:, :-1, :]
            targ = ids[:, 1:]
...
def run_theorem1_experiment(
    ...
    max_tests=100,
    device=None,
    log_r_pool=None,      # Assumption 2 的 log_r 样本池
    quant_c1=0.05,
    quant_c2=0.01
) -> Dict[str, object]:
    if device is None:
    ...
    log_c1_hat = estimate_log_c1_from_pool(log_r_pool or [], quantile=quant_c1)
    log_c2_hat = estimate_log_c2_per_token(model, tok, trec, phi_task, sample_prompts=32, device=device, q=quant_c2)
...
if __name__ == "__main__":
    ...
    for phi_star in tasks:
        ...
        c1_stats_blind, log_r_pool_blind = estimate_c1_independence(
        ...
        lp_stats = estimate_c2_prompt_likelihodd(trec, phi_star, model, tok, R=50)
        ...
        try:
            ...
            ys_conc = [curve[k] for k in xs]
            ys_probe = [probes[i]["probe_acc"] for i, _ in enumerate(xs)]
            # Theorem1 per-k 成功率（没有 k=0 的话补个 0）
            theo_map = theorem1["per_k_success"]
            ys_theo = [theo_map.get(int(k), 0.0) for k in xs]
            ...
            plt.title(f"{phi_star.name}: Lemma1 vs Probe vs Theorem1 (ε={EPSILON})\n"
                      f"log c1≈{theorem1['log_c1_hat']:.1f}, log c2≈{theorem1['log_c2_hat']:.1f}, θ≈{theorem1['theta']:.1f}")
            plt.tight_layout()
            png_name = f"lemma1_theorem1_{phi_star.name}.png"
